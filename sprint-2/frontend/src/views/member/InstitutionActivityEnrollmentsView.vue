<template>
  <v-card class="table">
    ...
    <participation-selection-dialog
      ...
      :enrollment-shifts="currentEnrollmentShifts"
...
export default class InstitutionActivityEnrollmentsView extends Vue {
  ...
  async selectParticipant(enrollment: Enrollment) {
    const activityId = enrollment.activityId;
    const volunteerId = enrollment.volunteerId;
    this.currentEnrollmentId = enrollment.id;

    this.currentEnrollmentShifts = this.activity.shifts.filter((shift) =>
      enrollment.shiftIds.includes(shift.id!),
    );
    ...
    if (existingParticipation) {
      ...
      this.currentParticipation.enrollmentShifts = enrollment.shiftIds;
      ...
      this.currentParticipation.enrollmentShifts = enrollment.shiftIds;
      this.editParticipationSelectionDialog = true;
    } else if (enrollment.shiftIds && enrollment.shiftIds.length > 0 && enrollment.id !== null) {
      await this.createParticipation(enrollment, enrollment.shiftIds[0]);
    } else {
      await this.$store.dispatch('error', new Error('Shift or enrollment id not found'));
    }
  }

  async createParticipation(enrollment: Enrollment, shiftId: number) {
    const activityId = enrollment.activityId;
    const volunteerId = enrollment.volunteerId;
    const enrollmentId = enrollment.id;

    if (!activityId || !volunteerId || !enrollmentId) {
      await this.$store.dispatch('error', new Error('Invalid participation data'));
      return;
    }
    ...
    try {
      await RemoteServices.createParticipation(shiftId, enrollmentId, this.currentParticipation);
      this.enrollments = await RemoteServices.getActivityEnrollments(activityId);
      this.participations = await RemoteServices.getActivityParticipations(activityId);
    } catch (error) {
      await this.$store.dispatch('error', error);
    } finally {
