...
class SpockTest extends Specification {
    ...
    public static final LocalDateTime FOUR_DAYS_AGO = DateHandler.now().minusDays(4)
    ...
    public static final LocalDateTime IN_FOUR_DAYS = DateHandler.now().plusDays(4)
    ...
    def createEnrollment(volunteer, shifts, motivation) {
        ...
        enrollmentDto.setShiftIds(shifts.collect { it.id })
    ...
    public static final String SHIFT_LOCATION_1 = "shift location 1 for testing purposes"
    public static final String SHIFT_LOCATION_2 = "shift location 2 for testing purposes"
    ...
    def createShift(activity, shiftDto) {
        def shift = new Shift(activity, shiftDto)
        shiftRepository.save(shift)
        return shift
    }

    def createShiftDto(location, start, end, participantsLimit) {
        def shiftDto = new ShiftDto()
        shiftDto.setLocation(location)
        shiftDto.setStartTime(DateHandler.toISOString(start))
        shiftDto.setEndTime(DateHandler.toISOString(end))
