...
class DeleteEnrollmentMethodTest extends SpockTest {
    ...
    def setup() {
        ...
        enrollmentOne = new Enrollment(volunteer, shifts, enrollmentDto)
    }

    def "get enrollments from activity via shifts"() {
        given: "a shift"
        def shiftDto = new ShiftDto()
        shiftDto.location = SHIFT_LOCATION_1
        shiftDto.startTime = DateHandler.toISOString(IN_TWO_DAYS)
        shiftDto.endTime = DateHandler.toISOString(IN_THREE_DAYS)
        shiftDto.participantsLimit = 2
        def shift = new Shift(activity, shiftDto)
        
        and: "an enrollment with shift"
        def enrollmentDto = new EnrollmentDto()
        enrollmentDto.motivation = ENROLLMENT_MOTIVATION_1
        def enrollment = new Enrollment(volunteer, [shift], enrollmentDto)
        shift.getEnrollments().add(enrollment)

        when:
        def enrollments = activity.getEnrollments()
